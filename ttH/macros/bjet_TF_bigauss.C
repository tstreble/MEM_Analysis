...
void plot_deltaE_nodRveto(){
  ...
  TString filename="bjet_bigauss_deltaE_barrel_nodRveto";
...
void plot_E_bin_nodRveto(bool low_E){
  ...
  h_E[0]->SetTitle("b-jet energy resolution tt+jets sample");
  ...
  TString filename="bjet_bigauss_Ebin_barrel_nodRveto";
  if(low_E)
    filename+="_low_E";
  else
    filename+="_high_E";
...
void plot_mean_sigma_jetTF(){
  ...
  for(int i=0;i<E_cut.size();i++){
    TH1F* h_nodRveto = single_plot(file_TTJets,"HTauTauTree",var,cut+E_cut[i],90,-180,180);
  ...
  TString filename0="frac_bjetTF_bigauss";
  ...
  TString filename="mean_bjetTF_bigauss";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_bjetTF_bigauss";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("bjetTF_bigauss_TTjets.root","RECREATE");
...
void plot_mean_sigma_jetTF_fixfrac(){
  ...
  TString filename="mean_bjetTF_bigauss_fixfrac";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_bjetTF_bigauss_fixfrac";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("bjetTF_bigauss_fixfrac_TTjets.root","RECREATE");
  frac_nodRveto->SetName("frac_nodRveto");
  frac_nodRveto->Write();
  mean1_nodRveto->SetName("mean1_nodRveto");
  mean1_nodRveto->Write();
...
void plot_mean_fit(){
  ...
  TFile* f = TFile::Open("bjetTF_bigauss_fixfrac_TTjets.root");
  ...
  TF1* new_TF_mean1=new TF1("mean_new1", "[0]+[1]*x",65,250);
  new_TF_mean1->SetParameter(0,-4.1);
  new_TF_mean1->SetParameter(1,1.03); 
  mean1_nodRveto->Fit(new_TF_mean1,"R0");
  new_TF_mean1->SetLineColor(3);

  TF1* new_TF_mean2=new TF1("mean_new2", "[0]+[1]*x",65,250);
  new_TF_mean2->SetParameter(0,-3.8);
  new_TF_mean2->SetParameter(1,0.95);
  ...
  TString filename="mean_bjetTF_bigauss_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_sigma_fit(){
  ...
  TF1* new_TF_sigma1=new TF1("sigma_new1", "TMath::Sqrt(([0]**2+([1]*TMath::Sqrt(x))**2)+([2]*x)**2)",60,250);
  new_TF_sigma1->SetParameter(0,0);
  new_TF_sigma1->SetParLimits(0,0,500);
  new_TF_sigma1->SetParameter(1,1.1);
  new_TF_sigma1->SetParLimits(1,0,500);
  new_TF_sigma1->SetParameter(2,0);
  sigma1_nodRveto->Fit(new_TF_sigma1,"R0");
  new_TF_sigma1->SetLineColor(3);

  TF1* new_TF_sigma2=new TF1("sigma_new2", "TMath::Sqrt(([0]**2+([1]*TMath::Sqrt(x))**2)+([2]*x)**2)",60,250);
  new_TF_sigma2->SetParameter(0,0);
  new_TF_sigma2->SetParameter(1,2.2);
  new_TF_sigma2->SetParLimits(1,0,500);
  new_TF_sigma2->SetParameter(2,0.1);
  ...
  TString filename="sigma_bjetTF_bigauss_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
