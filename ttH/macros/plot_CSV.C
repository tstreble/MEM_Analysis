...
void plot_CSVmax_2lSS(){

  vector<TH1F*> h;

  h.push_back(single_plot("/data_CMS/cms/strebler/ttH_Samples/ntuples_converted/MiniAODv2_prod_12_2015/ntuple_ttH_dRveto_gen_iso70.root","HTauTauTree","recoPFJet30_CSVsort_CSVscore[0]","MC_weight*PUReweight*(n_recolep==2&&recolep_charge[0]*recolep_charge[1]>0&&n_recotauh>0&&recotauh_charge[0]*recolep_charge[0]<0&&recoPFJet30_CSVsort_CSVscore[0]>0)",23,0,1.1));
  h.push_back(single_plot("/data_CMS/cms/strebler/ttH_Samples/ntuples_converted/MiniAODv2_prod_12_2015/ntuple_ttH_dRveto_gen_iso70.root","HTauTauTree","recoPFJet30_CSVsort_CSVscore[1]","MC_weight*PUReweight*(n_recolep==2&&recolep_charge[0]*recolep_charge[1]>0&&n_recotauh>0&&recotauh_charge[0]*recolep_charge[0]<0&&recoPFJet30_CSVsort_CSVscore[1]>0)",23,0,1.1));
  h.push_back(single_plot("/data_CMS/cms/strebler/ttH_Samples/ntuples_converted/MiniAODv2_prod_12_2015/ntuple_ttH_dRveto_gen_iso70.root","HTauTauTree","recoPFJet30_CSVsort_CSVscore[2]","MC_weight*PUReweight*(n_recolep==2&&recolep_charge[0]*recolep_charge[1]>0&&n_recotauh>0&&recotauh_charge[0]*recolep_charge[0]<0&&recoPFJet30_CSVsort_CSVscore[2]>0)",23,0,1.1));


  vector<TString> leg_entry;
  leg_entry.push_back("Jet with highest CSV");
  leg_entry.push_back("2^{nd} jet with highest CSV");
  leg_entry.push_back("3^{rd} jet with highest CSV");


  TLegend* leg=new TLegend(0.2,0.65,0.5,0.85);
  leg->SetHeader("2 same-sign leptons category");
  leg->SetBorderSize(0);
  ...
  TString filename="CSVmax_ttH_2lSS";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
