...
void datacard_maker(TString var, int nbin, float xmin, float xmax,
  ...
  for(unsigned int i_cat=0;i_cat<cat_names.size();i_cat++){
    ...
    for(unsigned i_MC=0;i_MC<MC_sample_name.size();i_MC++){
      ...
      float norm = h_MC->Integral();
      if(norm>0){
	for(unsigned int ibin=1;ibin<=h_MC->GetNbinsX();ibin++){
	  float bin_content=h_MC->GetBinContent(ibin);
	  if(bin_content<0){
	    cout<<MC_sample_name[i_MC]<<endl;
	    cout<<"Negative bin="<<h_MC->GetBinContent(ibin)<<endl;
	    h_MC->SetBinContent(ibin,1e-6);	 
	    h_MC->SetBinError(ibin,5*fabs(bin_content));
	  }
	}
	
	h_MC->Scale(norm/h_MC->Integral());
      }
   ...
   for(unsigned int i=0;i<bkg_type.size();i++){
    ...
    float norm_fakes = h_fakes->Integral();
    for(unsigned int ibin=1;ibin<=h_fakes->GetNbinsX();ibin++){
	float bin_content=h_fakes->GetBinContent(ibin);
	if(bin_content<0){
	  cout<<"fakes"<<endl;
	  cout<<"Negative bin="<<h_fakes->GetBinContent(ibin)<<endl;
	  h_fakes->SetBinContent(ibin,1e-6);	 
	  h_fakes->SetBinError(ibin,5*fabs(bin_content));
	}
      }
    h_fakes->Scale(norm_fakes/h_fakes->Integral());
    ...
    TH1F* h_ttbar_DL_fakes_MC =single_plot(filename_ttbar_DL,
				  "T",
				  var,
				  Form("MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*(1-0.1*recotauh_sel_isGenMatched[0])*event_weight_ttH*(category==%i && isTrig && recotauh_sel_byMediumIsolationMVArun2v1DBdR03oldDMwLT[0] && ",cat_fakes[i_cat]) + cut_cat + ")",
				  nbin,xmin,xmax);
    ...
    float norm_flips = h_flips->Integral();
    for(unsigned int ibin=1;ibin<=h_flips->GetNbinsX();ibin++){
	float bin_content=h_flips->GetBinContent(ibin);
	if(bin_content<0){
	  cout<<"flips"<<endl;
	  cout<<"Negative bin="<<h_flips->GetBinContent(ibin)<<endl;
	  h_flips->SetBinContent(ibin,1e-6);	 
	  h_flips->SetBinError(ibin,5*fabs(bin_content));
	}
      }
    h_flips->Scale(norm_flips/h_flips->Integral());
	...
	float norm = h_MC_btag_syst->Integral();
	if(norm>0){
	for(unsigned int ibin=1;ibin<=h_MC_btag_syst->GetNbinsX();ibin++){
	  float bin_content = h_MC_btag_syst->GetBinContent(ibin);
	  if(bin_content<0){
	    /*cout<<"btag syst"<<endl;
	    cout<<MC_sample_name[i_MC]<<endl;
	    cout<<"Negative bin="<<h_MC_btag_syst->GetBinContent(ibin)<<endl;*/
	    h_MC_btag_syst->SetBinContent(ibin,1e-6);	 
	    h_MC_btag_syst->SetBinError(ibin,5*fabs(bin_content));
	  }
	}
	...
	syst_name+="ttW";
      ...
      if(filename_MC_thu_unc[i_MC].Contains("12_2016"))
	 cut = "PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*(1-0.1*recotauh_sel_isGenMatched[0])";
      ...
      float norm = h_MC_x1_up->Integral();
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_x1_up->GetNbinsX();ibin++){
	float bin_content = h_MC_x1_up->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"theor syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_x1_up->GetBinContent(ibin)<<endl;*/
	  h_MC_x1_up->SetBinContent(ibin,1e-6);	 
	  h_MC_x1_up->SetBinError(ibin,5*fabs(bin_content));
	}
      }
      ...
      norm = h_MC_x1_down->Integral();
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_x1_down->GetNbinsX();ibin++){
	float bin_content = h_MC_x1_down->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"theor syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_x1_down->GetBinContent(ibin)<<endl;*/
	  h_MC_x1_down->SetBinContent(ibin,1e-6);	 
	  h_MC_x1_down->SetBinError(ibin,5*fabs(bin_content));
	}
      }
      ...
      norm = h_MC_y1_up->Integral();      
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_y1_up->GetNbinsX();ibin++){
	float bin_content = h_MC_y1_up->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"theor syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_y1_up->GetBinContent(ibin)<<endl;*/
	  h_MC_y1_up->SetBinContent(ibin,1e-6);	 
	  h_MC_y1_up->SetBinError(ibin,5*fabs(bin_content));
	}
      }      
      ...
      norm = h_MC_y1_down->Integral();      
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_y1_down->GetNbinsX();ibin++){
	float bin_content = h_MC_y1_down->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"theor syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_y1_down->GetBinContent(ibin)<<endl;*/
	  h_MC_y1_down->SetBinContent(ibin,1e-6);	 
	  h_MC_y1_down->SetBinError(ibin,5*bin_content);
	}
      }      
  ...
  cout<<"OK syst theoretical "<<endl;
    ...
    for(unsigned i_MC=0;i_MC<MC_sample_name.size();i_MC++){
      ...
      TString cut = "MC_weight2*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*(1-0.1*recotauh_sel_isGenMatched[0])";
      if(filename_MC[i_MC].Contains("ttH_prod_80X_12_2016"))
	cut = "MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*(1-0.1*recotauh_sel_isGenMatched[0])";
      ...
      float norm = h_MC_JEC_up->Integral();      
      if(norm>0){      
      for(unsigned int ibin=1;ibin<=h_MC_JEC_up->GetNbinsX();ibin++){
	float bin_content = h_MC_JEC_up->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"JECup syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_JEC_up->GetBinContent(ibin)<<endl;*/
	    h_MC_JEC_up->SetBinContent(ibin,1e-6);	 
	    h_MC_JEC_up->SetBinError(ibin,5*bin_content);
	}
      }         
      ...
      norm = h_MC_JEC_down->Integral();      
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_JEC_down->GetNbinsX();ibin++){
	float bin_content = h_MC_JEC_down->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"JECdown syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_JEC_down->GetBinContent(ibin)<<endl;*/
	  h_MC_JEC_down->SetBinContent(ibin,1e-6);	 
	  h_MC_JEC_down->SetBinError(ibin,5*fabs(bin_content));
	}
      }      
      ...
      norm = h_MC_TES_up->Integral();      
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_TES_up->GetNbinsX();ibin++){
	float bin_content = h_MC_TES_up->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"TESup syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_TES_up->GetBinContent(ibin)<<endl;*/
	  h_MC_TES_up->SetBinContent(ibin,1e-6);	 
	  h_MC_TES_up->SetBinError(ibin,5*fabs(bin_content));
	}
      }      
      ...
      norm = h_MC_TES_down->Integral();      
      if(norm>0){
      for(unsigned int ibin=1;ibin<=h_MC_TES_down->GetNbinsX();ibin++){
	float bin_content = h_MC_TES_down->GetBinContent(ibin);
	if(bin_content<0){
	  /*cout<<"TESdown syst"<<endl;
	  cout<<MC_sample_name[i_MC]<<endl;
	  cout<<"Negative bin="<<h_MC_TES_down->GetBinContent(ibin)<<endl;*/
	  h_MC_TES_down->SetBinContent(ibin,1e-6);	 
	  h_MC_TES_down->SetBinError(ibin,5*fabs(bin_content));
	}
      }      
...
void datacard_maker_MEM_ttH_vs_full_bkg_2lSS_nomiss(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MEM_ttH_vs_full_bkg_2lSS_nomiss_nofaketau_12.9fb_112016.root";
...
void datacard_maker_MEM_ttH_vs_full_bkg_2lSS_missing(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MEM_ttH_vs_full_bkg_2lSS_missing_nofaketau_12.9fb_112016.root";
...
void datacard_maker_MEM_ttH_vs_full_bkg_2lSS_nomiss_missing(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MEM_ttH_vs_full_bkg_2lSS_nomiss_missing_nofaketau_12.9fb_112016.root";
...
void datacard_maker_MEM_ttH_vs_full_bkg_2lSS_nomiss_njet4(){
  ...
  TString cut="integration_type==0 && n_recoPFJet>=4";

  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MEM_ttH_vs_full_bkg_2lSS_nomiss_nofaketau_12.9fb_112016_njet4.root";
...
void datacard_maker_MEM_ttH_vs_full_bkg_2lSS_missing_njet4(){
  ...
  TString cut="integration_type==1 && n_recoPFJet>=4";

  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MEM_ttH_vs_full_bkg_2lSS_missing_nofaketau_12.9fb_112016_njet4.root";
...
void datacard_maker_MEM_ttH_vs_full_bkg_2lSS_nomiss_missing_njet4(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MEM_ttH_vs_full_bkg_2lSS_nomiss_missing_nofaketau_12.9fb_112016_njet4.root";
