...
void plot_deltapT_nodRveto(){
  ...
  TString cut="recoPFJet30_dR_closest_genpart<0.4 && (abs(genpart_pdg[recoPFJet30_i_closest_genpart])<5) && abs(recoPFJet30_eta)>3 && genpart_pt[recoPFJet30_i_closest_genpart]>20";    

 
  TH1F* h =single_plot(file_VBF,"HTauTauTree",var,cut,100,-200,200);
  ...
  TString filename="jet_bigauss_deltapT_HF_nodRveto";
...
void plot_pT_bin_nodRveto(bool low_pT){
  ...
  for(unsigned int i=0;i<pT_cut.size();i++){
    h_pT.push_back(single_plot(file_VBF,"HTauTauTree",var_pT,cut+pT_cut[i],90,0,360));
    h_deltapT.push_back(single_plot(file_VBF,"HTauTauTree",var_deltapT,cut+pT_cut[i],90,-180,180));
  }
  ...
  TString filename="jet_bigauss_pTbin_HF_nodRveto";
  if(low_pT)
    filename+="_low_pT";
  else
    filename+="_high_pT";
...
void plot_mean_sigma_jetTF(){
  ...
  for(unsigned int i=0;i<pT_cut.size();i++){
    TH1F* h_nodRveto = single_plot(file_VBF,"HTauTauTree",var,cut+pT_cut[i],90,-180,180);
  ...
  TString filename0="frac_jetTF_HF_bigauss_pT";
  ...
  TString filename="mean_jetTF_HF_bigauss_pT";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_HF_bigauss_pT";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_HF_bigauss_pT_TTjets.root","RECREATE");
...
void plot_mean_sigma_jetTF_fixfrac(){
  ...
  TString filename="mean_jetTF_HF_bigauss_pT_fixfrac";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_HF_bigauss_pT_fixfrac";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_HF_bigauss_pT_fixfrac_TTjets.root","RECREATE");
...
void plot_mean_fit(){
  ...
  TFile* f = TFile::Open("jetTF_HF_bigauss_pT_fixfrac_TTjets.root");
  ...
  TF1* new_TF_mean1=new TF1("mean_new1", "[0]+[1]*x",60,210);
  ...
  TF1* new_TF_mean2=new TF1("mean_new2", "[0]+[1]*x",60,210);
  ...
  TString filename="mean_jetTF_HF_bigauss_pT_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_sigma_fit(){
  ...
  TFile* f = TFile::Open("jetTF_HF_bigauss_pT_TTjets.root");
  ...
  TF1* new_TF_sigma1=new TF1("sigma_new1", "TMath::Sqrt(([0]**2+([1]*TMath::Sqrt(x))**2)+([2]*x)**2)",60,210);
  ...
  TF1* new_TF_sigma2=new TF1("sigma_new2", "TMath::Sqrt(([0]**2+([1]*TMath::Sqrt(x))**2)+([2]*x)**2)",60,210);
  ...
  TString filename="sigma_jetTF_HF_bigauss_pT_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
