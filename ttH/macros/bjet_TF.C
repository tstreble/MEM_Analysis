...
void plot_E_bin_dRveto(bool low_E){
  ...
  TString cut="recoPFJet30_dR_closest_genpart<0.4 && recoPFJet30_dR_2nd_closest_genpart>0.4 && abs(genpart_pdg[recoPFJet30_i_closest_genpart])==5 && abs(recoPFJet30_eta)<1.5";  
  ...
  for(int i=0;i<E_cut.size();i++){
    ...
    TF1* f=new TF1("f","4*(1/([1]*sqrt(2*pi))) * exp(-0.5*((x-[0])/[1])**2)",30,360);
    fit.push_back(f);
  ...
  TString filename="bjet_Ebin_barrel_dRveto";
  if(low_E)
    filename+="_low_E";
  else
    filename+="_high_E";
...
void plot_E_bin_nodRveto(bool low_E){
  ...
  for(int i=0;i<E_cut.size();i++){
    ...
    fit_double[i]->SetParameter(4,h[i]->GetRMS());
    fit_double[i]->SetParLimits(0,0,1);
    fit_double[i]->FixParameter(0,0.67);
  ...
  TString filename="bjet_Ebin_barrel_nodRveto";
  if(low_E)
    filename+="_low_E";
  else
    filename+="_high_E";
...
void plot_deltaE_nodRveto(){
  ...
  TString filename="bjet_deltaE_barrel_nodRveto";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_mean_sigma_jetTF(){
  ...
  for(int i=0;i<E_cut.size();i++){
    ...
    fit_nodRveto[i]->FixParameter(0,0.67);
  ...
  TString filename2="sigma_bjetTF";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("bjetTF_TTjets.root","RECREATE");
...
void plot_mean_sigma_jetTF_free_relfrac(){
  ...
  for(int i=0;i<E_cut.size();i++){
    ...
    fit_nodRveto[i]->SetParameter(4,h_nodRveto[i]->GetRMS());
    fit_nodRveto[i]->SetParLimits(0,0,1);
    //fit_nodRveto[i]->FixParameter(0,0.65);
  ...
  TFile* f_new = TFile::Open("bjetTF_TTjets_free_relfrac.root","RECREATE");
  frac_nodRveto->SetName("frac_nodRveto");
  frac_nodRveto->Write();
  mean1_nodRveto->SetName("mean1_nodRveto");
  mean1_nodRveto->Write();
  mean2_nodRveto->SetName("mean2_nodRveto");
  mean2_nodRveto->Write();
  sigma1_nodRveto->SetName("sigma2_nodRveto");
  sigma1_nodRveto->Write();
  sigma2_nodRveto->SetName("sigma2_nodRveto");
  sigma2_nodRveto->Write();
  f_new->Close();
...
void plot_mean_fit(){
  ...
  TFile* f = TFile::Open("bjetTF_TTjets.root");
  ...
  old_TF_mean1->FixParameter(0,-4.1);
  old_TF_mean1->FixParameter(1,1.03);
  TF1* old_TF_mean2=new TF1("mean_old2", "[0]+[1]*x",0,350);
  old_TF_mean2->FixParameter(0,-3.8);
  old_TF_mean2->FixParameter(1,0.95);
  old_TF_mean2->SetLineColor(4);*/


  TF1* new_TF_mean1=new TF1("mean_new1", "[0]+[1]*x",60,250);
  ...
  TF1* new_TF_mean2=new TF1("mean_new2", "[0]+[1]*x",60,250);
  ...
  TString filename="mean_bjetTF_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_sigma_fit(){
  ...
  old_TF_sigma1->FixParameter(1,1.1);
  ...
  old_TF_sigma2->FixParameter(1,2.2);
  old_TF_sigma2->FixParameter(2,0.1);
  ...
  TString filename="sigma_bjetTF_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
