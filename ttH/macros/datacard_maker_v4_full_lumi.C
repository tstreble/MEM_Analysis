...
void datacard_maker(TString var, int nbin, float xmin, float xmax,
  ...
  for(unsigned int i_cat=0;i_cat<cat_names.size();i_cat++){
    ...
    TH1F* h_ttbar_DL_fakes_MC =single_plot(filename_ttbar_DL,
				  "HTauTauTree_2lSS_lepMVA_CR",
				  var,
				  Form("MC_weight*PU_weight*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*event_weight_ttH*(category==%i && isTrig)",cat_fakes[i_cat]),
				  nbin,xmin,xmax);
...
void datacard_maker_MVA_2lSS_allcat_syst(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MVA_2lSS_allcat_syst_27.656fb_112016.root";
