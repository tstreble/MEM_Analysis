...
TString filename_ttW = "/data_CMS/cms/strebler/ttH_prod_80X_11_2016/ntuples_splitted/nominal/ttV/HTauTauTree_ttW*_split_VTightTau_3l.root";
  ...
  for(unsigned int i_cat=0;i_cat<cat_names.size();i_cat++){
    ...
    for(unsigned i_MC=0;i_MC<MC_sample_name.size();i_MC++){
      ...
      TH1F* h_MC_genmatch = single_plot(filename_MC[i_MC],
					...
					var,
					Form("MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight_v2*bTagSF_CSVshape_weight*(1-0.1*recotauh_sel_isGenMatched[0])*event_weight_ttH_v2*(category_v2==%i && isTrig_3l && recotauh_sel_byVTightIsolationMVArun2v1DBdR03oldDMwLT[0] &&",cat_fakes[i_cat]) + MC_sample_cut[i_MC] + " && " + cut_cat + ")",
					nbin,xmin,xmax);
    ...
    TH1F* h_fakes =single_plot(filename_data,
			       "HTauTauTree_3l_lepMVA_CR",
			       var,
			       Form("event_weight_ttH_v2*(category==%i && isTrig_3l && recotauh_sel_byVTightIsolationMVArun2v1DBdR03oldDMwLT[0] && ",cat_fakes[i_cat]) + cut_cat + ")",
			       nbin,xmin,xmax);
...
void datacard_maker_MVA_3l_syst(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MVA_3l1tau_syst_nofaketau_VTightTau_12.9fb_112016.root";
