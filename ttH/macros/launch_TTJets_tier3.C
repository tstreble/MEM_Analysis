...
void create_scripts(){
  ...
  for(unsigned int i=1;i<10;i++){
    for(unsigned int j=1;j<5;j++){
      ...
      TString ScriptName = Form("script_tier3/convert_TTJets_%i_%i.sh",i,j);
      ofstream myscript;
      ...
      myscript << "convert_tree(\"TTJets_" << i << "\",70,4,"<< j << ");\n";
      myscript << ".q\n";
      myscript << "EOF\n";
      myscript.close();   
...
void launch_scripts(){
  ...
  for(unsigned int i=1;i<10;i++){
    for(unsigned int j=1;j<5;j++){
      ...
      LaunchExec += Form("script_tier3/convert_TTJets_%i_%i.sh",i,j);
