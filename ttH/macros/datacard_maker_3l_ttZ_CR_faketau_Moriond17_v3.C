 ...
 TString var = var1 + "*(" + var1 + Form("<=%f) + %f*(",xmax,0.999*xmax) + var1 + Form(">%f)",xmax);
  ...
  for(unsigned int i_cat=0;i_cat<cat_names.size();i_cat++){
    ...
    TH1F* h_fakes =single_plot(filename_data,
			       "HTauTauTree_3l_ttZ_lepMVA_CR",
			       var,
			       "event_weight_ttH_v2*(isTrig_3l_v2 && " + cut_cat + ")",
			       nbin,xmin,xmax);
...
void datacard_maker_n_recotauh_3l_ttZ_CR(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_01_2017/datacards/datacard_n_recotauh_3l_ttZ_CR_faketau_syst_35.9fb_022017.root";
