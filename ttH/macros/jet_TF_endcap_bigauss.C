...
void plot_deltaE_nodRveto(){
  ...
  TString filename="jet_bigauss_deltaE_endcap_nodRveto";
...
void plot_deltaE_gluon_nodRveto(){
  ...
  TString var="recoPFJet30_e-genpart_e[recoPFJet30_i_closest_genpart]";
  TString cut="recoPFJet30_dR_closest_genpart<0.4 && (abs(genpart_pdg[recoPFJet30_i_closest_genpart])==21) && abs(recoPFJet30_eta)>1.5 && abs(recoPFJet30_eta)<3";    

 
  TH1F* h =single_plot(file_TTJets,"HTauTauTree",var,cut,100,-100,500);
  ...
  TString filename="jet_bigauss_gluon_deltaE_endcap_nodRveto";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_E_bin_nodRveto(bool low_E){
  ...
  for(int i=0;i<E_cut.size();i++){
    ...
    //TF1* f=new TF1("f",double_gauss_order,-180,180,5);
    ...
    if(f->GetParameter(1)>f->GetParameter(3)){
      ...
      f2->SetParameter(2,f->GetParameter(0)+E_mean_bin[i]);
      f2->SetParameter(3,f->GetParameter(1));
      f2->SetParameter(0,f->GetParameter(2)+E_mean_bin[i]);
  ...
  TString filename="jet_bigauss_Ebin_endcap_nodRveto";
  if(low_E)
    filename+="_low_E";
  else
    filename+="_high_E";
...
void plot_mean_sigma_jetTF(){
  ...
  TString filename0="frac_jetTF_bigauss_endcap";
  ...
  TString filename="mean_jetTF_bigauss_endcap";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_bigauss_endcap";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_bigauss_endcap_TTjets.root","RECREATE");
...
void plot_mean_sigma_jetTF_fixfrac(){
  ...
  TString filename="mean_jetTF_bigauss_fixfrac_endcap";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_bigauss_fixfrac_endcap";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_bigauss_fixfrac_endcap_TTjets.root","RECREATE");
  frac_nodRveto->SetName("frac_nodRveto");
  frac_nodRveto->Write();
  mean1_nodRveto->SetName("mean1_nodRveto");
  mean1_nodRveto->Write();
...
void plot_mean_fit(){
  ...
  TFile* f = TFile::Open("jetTF_bigauss_fixfrac_endcap_TTjets.root");
  ...
  TString filename="mean_jetTF_endcap_bigauss_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_sigma_fit(){
  ...
  TLegend* leg=new TLegend(0.3,0.6,0.5,0.87);
  leg->SetBorderSize(0);
  leg->SetTextSize(0.035);
  leg->SetFillColor(0);
  ...
  sigma1_nodRveto->GetYaxis()->SetRangeUser(0,120);
  sigma1_nodRveto->GetXaxis()->SetTitle("E_{q} [GeV]");
  sigma1_nodRveto->GetYaxis()->SetTitle("#sigma [GeV]");
  sigma1_nodRveto->GetYaxis()->SetTitleOffset(1.6);
  ...
  TString filename="sigma_jetTF_endcap_bigauss_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
