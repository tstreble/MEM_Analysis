...
void datacard_maker(TString var, int nbin, float xmin, float xmax,
  ...
  for(unsigned int i_cat=0;i_cat<cat_names.size();i_cat++){
	...
	h_MC= single_plot(filename_MC[i_MC],
			  "HTauTauTree_2lSS",
			  var,
			  Form("MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight_v2*bTagSF_CSVshape_weight*(1-0.1*recotauh_sel_isGenMatched[0])*(category==%i && isTrig &&",cat[i_cat]) + MC_sample_cut[i_MC] + " && " + cut_cat + ")",
			  nbin,xmin,xmax);
	...
	h_MC_genmatch = single_plot(filename_MC[i_MC],
				    ...
				    var,
				    Form("MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight_v2*bTagSF_CSVshape_weight*event_weight_ttH*(1-0.1*recotauh_sel_isGenMatched[0])*(category==%i && isTrig && recotauh_sel_byMediumIsolationMVArun2v1DBdR03oldDMwLT[0] && isGenMatched && ",cat_fakes[i_cat]) + MC_sample_cut[i_MC] + " && " + cut_cat + ")",
				    nbin,xmin,xmax);
      else if(filename_MC[i_MC].Contains("ttH_prod_80X_12_2016")) 
	h_MC_genmatch = single_plot(filename_MC[i_MC],
				    "HTauTauTree_2lSS_lepMVA_CR",
				    var,
				    Form("MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight*bTagSF_CSVshape_weight*event_weight_ttH*(1-0.1*recotauh_sel_isGenMatched[0])*(category==%i && isTrig && recotauh_sel_byMediumIsolationMVArun2v1DBdR03oldDMwLT[0] && isGenMatched && ",cat_fakes[i_cat]) + MC_sample_cut[i_MC] + " && " + cut_cat + ")",
				    nbin,xmin,xmax);
   ...
   bkg_type.erase(bkg_type.begin()+3);
    ...
    makeBinContentsPositive(h_ttbar_DL_fakes_MC);
	...
	if(filename_MC[i_MC].Contains("ttH_prod_80X_12_2016"))
	    cut = "MC_weight*PU_weight_ICHEP*triggerSF_weight*leptonSF_ttH_weight*(1-0.1*recotauh_sel_isGenMatched[0])*bTagSF_CSVshape_weight_" + bTag_syst_names[i_b];
...
TString h_name_ttZ = "x_ttZ"+bkg_type[i_bkg]+"_CMS_ttHl_btag_"+bTag_syst_names[i_b];
   ...
   vector<TString> thu_uncert;
    ...
    for(unsigned i_MC=0;i_MC<MC_sample_name.size();i_MC++){
      ...
      h_MC_JEC_up->SetNameTitle("x_"+MC_sample_name[i_MC]+"_CMS_ttHl_JESUp","x_"+MC_sample_name[i_MC]+"_JECUp");
	...
	if(bin_content<0){
      ...
      h_MC_JEC_down->SetNameTitle("x_"+MC_sample_name[i_MC]+"_CMS_ttHl_JESDown","x_"+MC_sample_name[i_MC]+"_JECDown");
      h_MC_JEC_down->Write();
	...
	if(bin_content<0){
      ...
      h_MC_TES_up->SetNameTitle("x_"+MC_sample_name[i_MC]+"_CMS_ttHl_tauESUp","x_"+MC_sample_name[i_MC]+"_TESUp");
      h_MC_TES_up->Write();
	...
	if(bin_content<0){
      ...
      h_MC_TES_down->SetNameTitle("x_"+MC_sample_name[i_MC]+"_CMS_ttHl_tauESDown","x_"+MC_sample_name[i_MC]+"_TESDown");
      h_MC_TES_down->Write();
...
void datacard_maker_MVA_2lSS_allcat_syst(){
  ...
  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MVA_2lSS_allcat_syst_nofaketau_12.9fb_112016.root";
...
void datacard_maker_MVA_2lSS_allcat_syst_njet4(){
  ...
  TString cut = "n_recoPFJet>=4";


  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MVA_2lSS_allcat_syst_nofaketau_12.9fb_112016_njet4.root";
...
void datacard_maker_MVA_2lSS_allcat_syst_TightTau(){
  ...
  TString cut = "recotauh_sel_byTightIsolationMVArun2v1DBdR03oldDMwLT[0]";


  TString file="/data_CMS/cms/strebler/ttH_prod_80X_11_2016/datacards/datacard_MVA_2lSS_allcat_syst_nofaketau_TightTau_12.9fb_112016.root";
