...
void plot_deltapT_nodRveto(){
  ...
  TString filename="jet_bigauss_deltapT_endcap_nodRveto";
...
void plot_pT_bin_nodRveto(bool low_pT){
  ...
  for(unsigned int i=0; i<h_pT.size();i++){
    ...
    h_deltapT[i]->SetLineColor(h_pT[i]->GetLineColor());
  ...
  for(unsigned int i=0;i<pT_cut.size();i++){
    ...
    fit_double[i]->SetLineColor(h_deltapT[i]->GetLineColor());
  ...
  TString filename="jet_bigauss_pTbin_endcap_nodRveto";
  if(low_pT)
    filename+="_low_pT";
  else
    filename+="_high_pT";
...
void plot_mean_sigma_jetTF(){
  ...
  for(int i=30; i<250; i+=10){
    pT_cut.push_back(Form("&& genpart_e[recoPFJet30_i_closest_genpart]>%i && genpart_e[recoPFJet30_i_closest_genpart]<%i",i,i+10));   
  ...
  for(unsigned int i=0;i<pT_cut.size();i++){
    ...
    f->SetParameter(4,0.8);
    f->SetParLimits(4,0,1);

    f->SetParLimits(1,0,5000);
  ...
  TString filename0="frac_jetTF_endcap_bigauss_pT";
  ...
  TString filename="mean_jetTF_endcap_bigauss_pT";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_endcap_bigauss_pT";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_endcap_bigauss_pT_TTjets.root","RECREATE");
...
void plot_mean_sigma_jetTF_fixfrac(){
  ...
  TString filename="mean_jetTF_endcap_bigauss_pT_fixfrac";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_endcap_bigauss_pT_fixfrac";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_endcap_bigauss_pT_fixfrac_TTjets.root","RECREATE");
  frac_nodRveto->SetName("frac_nodRveto");
  frac_nodRveto->Write();
  mean1_nodRveto->SetName("mean1_nodRveto");
  mean1_nodRveto->Write();
...
void plot_frac_fit(){
  ...
  TFile* f = TFile::Open("jetTF_endcap_bigauss_pT_TTjets.root");
  TGraphErrors* frac_nodRveto = (TGraphErrors*)f->Get("frac_nodRveto");

  TF1* new_TF_frac=new TF1("frac", "[0]",110,250);
  new_TF_frac->SetParameter(0,0.1);
  frac_nodRveto->Fit(new_TF_frac,"R0");
  new_TF_frac->SetLineColor(3);
  ...
  frac_nodRveto->GetYaxis()->SetRangeUser(0,1);
  frac_nodRveto->GetXaxis()->SetTitle("p_{T} [GeV]");
  frac_nodRveto->GetYaxis()->SetTitle("f");
  frac_nodRveto->GetYaxis()->SetTitleOffset(1.6);
  frac_nodRveto->SetTitle("Light jet TF relative normalization");

  frac_nodRveto->Draw("AP");
  new_TF_frac->Draw("SAME");

  leg->Draw("same");

  TString filename="frac_jetTF_endcap_bigauss_pT_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_mean_fit(){
  ...
  TFile* f = TFile::Open("jetTF_endcap_bigauss_pT_fixfrac_TTjets.root");
  ...
  TString filename="mean_jetTF_endcap_bigauss_pT_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_sigma_fit(){
  ...
  new_TF_sigma1->SetParameter(0,5);
  ...
  TString filename="sigma_jetTF_endcap_bigauss_pT_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
