...
void plot_deltaE_nodRveto(){
  ...
  TString cut="recoPFJet30_dR_closest_genpart<0.4 && (abs(genpart_pdg[recoPFJet30_i_closest_genpart])<5) && abs(recoPFJet30_eta)<1.5";    
  ...
  h->SetTitle("Light jets from quarks tt+jets sample");
  ...
  leg->SetHeader(Form("#splitline{p_{T}(reco)>30 GeV, |#eta(reco)|<1.5}{f(#DeltaE)=%.2fG_{1}+%.2fG_{2}}",f->GetParameter(0),1-f->GetParameter(0)));
  leg->Draw("same");


  TString filename="jet_bigauss_deltaE_barrel_nodRveto";
...
void plot_deltaE_gluon_nodRveto(){
  ...
  TString var="recoPFJet30_e-genpart_e[recoPFJet30_i_closest_genpart]";
  TString cut="recoPFJet30_dR_closest_genpart<0.4 && (abs(genpart_pdg[recoPFJet30_i_closest_genpart])==21) && abs(recoPFJet30_eta)<1.5";    
  ...
  h->GetYaxis()->SetTitle(Form("Fraction of events / %.2f GeV",binning(h)));
  h->GetYaxis()->SetTitleOffset(1.7);
  h->SetTitle("Light jets from gluons tt+jets sample");
  ...
  TString filename="jet_bigauss_gluon_deltaE_barrel_nodRveto";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_E_bin_nodRveto(bool low_E){
  ...
  for(int i=0;i<E_cut.size();i++){
    h_E.push_back(single_plot(file_TTJets,"HTauTauTree",var_E,cut+E_cut[i],90,0,360));
    h_deltaE.push_back(single_plot(file_TTJets,"HTauTauTree",var_deltaE,cut+E_cut[i],90,-180,180));
  }
  ...
  h_E[0]->SetTitle("Light jet energy resolution tt+jets sample");
  ...
  TString filename="jet_bigauss_Ebin_barrel_nodRveto";
  if(low_E)
    filename+="_low_E";
  else
    filename+="_high_E";
...
void plot_mean_sigma_jetTF(){
  ...
  for(int i=0;i<E_cut.size();i++){
    h_nodRveto.push_back(single_plot(file_TTJets,"HTauTauTree",var,cut+E_cut[i],90,-180,180));
    
    h_nodRveto[i]->Sumw2();
    h_nodRveto[i]->Scale(1/h_nodRveto[i]->Integral());
  
    TF1* f=new TF1("f",double_gauss,-180,180,5);
    f->SetParameter(0,h_nodRveto[i]->GetMean());
    f->SetParameter(1,h_nodRveto[i]->GetRMS());
    f->SetParameter(2,h_nodRveto[i]->GetMean());
    f->SetParameter(3,h_nodRveto[i]->GetRMS());
    //f->FixParameter(4,0.66);

    f->SetParameter(4,0.66);
    ...
    if(i>0){
      ...
      fit_nodRveto[i]->SetParameter(1,y);
      sigma2_nodRveto->GetPoint(i-1,x,y);      
      fit_nodRveto[i]->SetParameter(3,y);
      frac_nodRveto->GetPoint(i-1,x,y);
      fit_nodRveto[i]->SetParameter(4,y);
    }
    ...
    frac_nodRveto->SetPoint(i,bin_mean[i],fit_nodRveto[i]->GetParameter(4));
    frac_nodRveto->SetPointError(i,bin_width[i],fit_nodRveto[i]->GetParError(4));
    mean1_nodRveto->SetPoint(i,bin_mean[i],fit_nodRveto[i]->GetParameter(0)+bin_mean[i]);
    mean1_nodRveto->SetPointError(i,bin_width[i],fit_nodRveto[i]->GetParError(0));
    sigma1_nodRveto->SetPoint(i,bin_mean[i],fit_nodRveto[i]->GetParameter(1));
    sigma1_nodRveto->SetPointError(i,bin_width[i],fit_nodRveto[i]->GetParError(1));
    mean2_nodRveto->SetPoint(i,bin_mean[i],fit_nodRveto[i]->GetParameter(2)+bin_mean[i]);
    mean2_nodRveto->SetPointError(i,bin_width[i],fit_nodRveto[i]->GetParError(2));
    sigma2_nodRveto->SetPoint(i,bin_mean[i],fit_nodRveto[i]->GetParameter(3));
    sigma2_nodRveto->SetPointError(i,bin_width[i],fit_nodRveto[i]->GetParError(3));
  ...
  TString filename0="frac_jetTF_bigauss";
  ...
  TString filename="mean_jetTF_bigauss";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
  ...
  TString filename2="sigma_jetTF_bigauss";
  c2->SaveAs("../plots/pdf/"+filename2+".pdf");
  c2->SaveAs("../plots/png/"+filename2+".png");


  TFile* f_new = TFile::Open("jetTF_bigauss_TTjets.root","RECREATE");
...
void plot_mean_sigma_jetTF_fixfrac(){
  ...
  TFile* f_new = TFile::Open("jetTF_bigauss_fixfrac_TTjets.root","RECREATE");
  frac_nodRveto->SetName("frac_nodRveto");
  frac_nodRveto->Write();
  mean1_nodRveto->SetName("mean1_nodRveto");
  mean1_nodRveto->Write();
...
void plot_mean_fit(){
  ...
  TFile* f = TFile::Open("jetTF_bigauss_fixfrac_TTjets.root");
  ...
  TF1* new_TF_mean1=new TF1("mean_new1", "[0]+[1]*x",50,250);
  ...
  TF1* new_TF_mean2=new TF1("mean_new2", "[0]+[1]*x",50,250);
  ...
  TString filename="mean_jetTF_bigauss_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
...
void plot_sigma_fit(){
  ...
  TF1* new_TF_sigma1=new TF1("sigma_new1", "TMath::Sqrt(([0]**2+([1]*TMath::Sqrt(x))**2)+([2]*x)**2)",50,250);
  ...
  TF1* new_TF_sigma2=new TF1("sigma_new2", "TMath::Sqrt(([0]**2+([1]*TMath::Sqrt(x))**2)+([2]*x)**2)",50,250);
  ...
  leg->AddEntry(new_TF_sigma1,Form("New TF #sigma_{1}: %.2f #oplus %.2f #sqrt{E_{q}} #oplus %.2f E_{q}",new_TF_sigma1->GetParameter(0),new_TF_sigma1->GetParameter(1),new_TF_sigma1->GetParameter(2)));
  leg->AddEntry(new_TF_sigma2,Form("New TF #sigma_{2}: %.2f #oplus %.2f #sqrt{E_{q}} #oplus %.2f E_{q}",new_TF_sigma2->GetParameter(0),new_TF_sigma2->GetParameter(1),new_TF_sigma2->GetParameter(2)));
  leg->SetHeader("p_{T}(reco)>30 GeV, |#eta(reco)|<1.5");
  ...
  TString filename="sigma_jetTF_bigauss_fit";
  c->SaveAs("../plots/pdf/"+filename+".pdf");
  c->SaveAs("../plots/png/"+filename+".png");
